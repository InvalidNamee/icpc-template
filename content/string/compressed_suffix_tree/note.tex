...
\begin{itemize}
  ...
  \item \texttt{t[v].suf}：叶子对应的后缀起点，内部结点为 $-1$；
  \item \texttt{t[v].rank}：叶子对应的 SA 排名，内部结点为 $-1$；
  \item \texttt{g[v]}：压缩树的儿子，顺序就是字符字典序；
  \item \texttt{leaf[i]}：后缀 $s[i\ldots n-1]$ 对应的叶子结点。
\end{itemize}
...
模式串直接传入普通字符串。\texttt{find(p)} 沿压缩边匹配，返回模式末尾所在边的下端结点；模式可能停在一条边中间，这不影响其子树表示全部出现。返回 $-1$ 表示不存在。

\texttt{occurrences(p)} 返回出现次数。\texttt{match(p)} 返回匹配后缀在 SA 中的 $0$ 下标闭区间 $[L,R]$；所有出现起点为
\[
