...
\begin{itemize}
  ...
  \item \texttt{pref[i]} 是原串前缀 $s[0\ldots i]$ 对应的状态。
\end{itemize}

\textbf{子串是否存在、出现多少次}

待查串 $p$ 直接传入普通字符串。\texttt{walk(p)} 沿转移走完，返回 $-1$ 表示 $p$ 不是原串子串，否则返回其状态。\texttt{occurrences(p)} 直接返回出现次数，允许重叠。
...
先对第一个串构造 SAM，再调用 \texttt{lcs(t)} 扫描第二个串。返回结构包含 \texttt{len}，以及该公共子串在原串中的 $0$ 下标闭区间 $[l1,r1]$ 和在 $t$ 中的 $[l2,r2]$；不存在公共字符时长度为 $0$，四个端点均为 $-1$。
